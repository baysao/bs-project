...
module.exports = function (req, res, next) {
    ...
    coll.findById(entryId, function (err, entry) {
        ...
        var file_encoded = {};
        if(typeof entry.file_encoded == 'string') {
            file_encoded = JSON.parse(entry.file_encoded);
        } else
            file_encoded = entry.file_encoded;

        var src = file_encoded[flavorId] && file_encoded[flavorId].src;
        console.log('src');
        console.log(src);
        if (!src) {
            res.sendStatus(404);
        ...
        var fullsrc = path.join(transcoded_dir, src);
        console.log(fullsrc);
