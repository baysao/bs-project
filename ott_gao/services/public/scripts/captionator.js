...
/* Modified by Ephraim Tabackman at Viewbix 2016-01-05 to not use built-in browser Text Track features. */
...
;(function(){
	...
	* @constructor
	*/
	captionator.CTextTrack = function CTextTrack(id,kind,label,language,trackSource,isDefault) {
		...
		this.id = id || "";
		this.internalMode = captionator.CTextTrack.OFF;
		this.cues = new captionator.CTextTrackCueList(this);
		this.activeCues = new captionator.ActiveCTextTrackCueList(this.cues,this);
		this.kind = kind || "subtitles";
		this.label = label || "";
		this.language = language || "";
		this.src = trackSource || "";
		this.readyState = captionator.CTextTrack.NONE;
		...
		this.setMode = function(value) {
			var allowedModes = [captionator.CTextTrack.OFF,captionator.CTextTrack.HIDDEN,captionator.CTextTrack.SHOWING], containerID, container;
			if (allowedModes.indexOf(value) !== -1) {
				if (value !== this.internalMode) {
					this.internalMode = value;
			
					if (this.readyState === captionator.CTextTrack.NONE && this.src.length > 0 && value > captionator.CTextTrack.OFF) {
					...
					if (value === captionator.CTextTrack.OFF) {
		...
		this.loadTrack = function(source, callback) {
			...
			if (this.readyState === captionator.CTextTrack.LOADED) {
				if (callback instanceof Function) {
					callback(captionData);
				}
			} else {
				this.src = source;
				this.readyState = captionator.CTextTrack.LOADING;
				...
				ajaxObject.onreadystatechange = function (eventData) {
					if (ajaxObject.readyState === 4) {
						if(ajaxObject.status === 200) {
							...
							currentTrackElement.readyState = captionator.CTextTrack.LOADED;
							...
							currentTrackElement.readyState = captionator.CTextTrack.ERROR;
		...
		// mutableCTextTrack.addCue(cue)
		// Adds the given cue to mutableCTextTrack's text track list of cues.
		...
		this.addCue = function(cue) {
			if (cue && cue instanceof captionator.CTextTrackCue) {
				this.cues.addCue(cue);
			} else {
				throw new Error("The argument is null or not an instance of CTextTrackCue.");
			}
		};
	
		// mutableCTextTrack.removeCue(cue)
		// Removes the given cue from mutableCTextTrack's text track list of cues.
	...
	// Define constants for CTextTrack.readyState
	captionator.CTextTrack.NONE = 0;
	captionator.CTextTrack.LOADING = 1;
	captionator.CTextTrack.LOADED = 2;
	captionator.CTextTrack.ERROR = 3;
	// Define constants for CTextTrack.mode
	captionator.CTextTrack.OFF = 0;
	captionator.CTextTrack.HIDDEN = 1;
	captionator.CTextTrack.SHOWING = 2;
	...
	* @constructor
	*/
	captionator.CTextTrackCue = function CTextTrackCue(id, startTime, endTime, text, settings, pauseOnExit, track) {
		// Set up internal data store
		this.id = id;
		this.track = track instanceof captionator.CTextTrack ? track : null;
		...
		this.isActive = function() {
			var currentTime = 0;
			if (this.track instanceof captionator.CTextTrack) {
				if ((this.track.mode === captionator.CTextTrack.SHOWING || this.track.mode === captionator.CTextTrack.HIDDEN) && this.track.readyState === captionator.CTextTrack.LOADED) {
					try {
	...
	* @constructor
	*/
	captionator.CTextTrackCueList = function CTextTrackCueList(track) {
		this.cTrack = track instanceof captionator.CTextTrack ? track : null;
		...
		this.loadCues = function(cueData) {
			for (var cueIndex = 0; cueIndex < cueData.length; cueIndex ++) {
			  cueData[cueIndex].track = this.cTrack;
		...
		this.addCue = function(cue) {
			if (cue && cue instanceof captionator.CTextTrackCue) {
			  if (cue.track === this.cTrack || !cue.track) {
					// TODO: Check whether cue is already in list of cues.
					// TODO: Sort cue list based on CTextTrackCue.startTime.
		...
		this.toString = function() {
			return "[CTextTrackCueList]";
		};
	};
	captionator.CTextTrackCueList.prototype = [];
	
	/**
	* @constructor
	*/
	captionator.ActiveCTextTrackCueList = function ActiveCTextTrackCueList(textTrackCueList,textTrack) {
		// Among active cues:
		...
		this.toString = function() {
			return "[ActiveCTextTrackCueList]";
		};
	
		this.refreshCues();
	};
	captionator.ActiveCTextTrackCueList.prototype = new captionator.CTextTrackCueList(null);
	...
	*/
	...
	/*
		...
		Loops through all the CTextTracks for a given element and manages their display (including generation of container elements.)
	
		First parameter: HTMLVideoElement object with associated CTextTracks
	...
	*/
	captionator.rebuildCaptions = function(videoElement) {
		var trackList = videoElement.cTextTracks || [];
		...
		trackList.forEach(function(track,trackIndex) {
			if (track.mode === captionator.CTextTrack.SHOWING && track.readyState === captionator.CTextTrack.LOADED) {
		...
		if (cuesChanged || videoElement._captionator_dirtyBit) {
			...
			compositeActiveCues.forEach(function(cue) {
				...
				if (cue.track.kind !== "metadata" && cue.mode !== captionator.CTextTrack.HIDDEN) {
	...
	*/
	captionator.captionify = function(element, defaultLanguage, options) {
	  ...
	  options = options instanceof Object ? options : { };
		...
		//if ((typeof(document.createElement("video").addCTextTrack) === "function" || typeof(document.createElement("video").addTrack) === "function") && !options.forceCaptionify) {
		//	return false;
		//}
		
		//// if requested by options, export the object types
		//if (!objectsCreated && options.exportObjects) {
		//	window.TextTrack = captionator.CTextTrack;
		//	window.TextTrackCueList = captionator.CTextTrackCueList;
		//	window.ActiveTextTrackCueList = captionator.ActiveCTextTrackCueList;
		//	window.TextTrackCue = captionator.CTextTrackCue;
		...
		if (videoElements.length) {
			videoElements.forEach(function(videoElement) {
				videoElement.addCTextTrack = function(id,kind,label,language,src,type,isDefault) {
					...
					if (!allowedKinds.filter(function (currentKind){
						...
						throw captionator.createDOMException(12,"DOMException 12: SYNTAX_ERR: You must use a valid kind when creating a TimedCTextTrack.","SYNTAX_ERR");
	
					} else {
						newTrack = new captionator.CTextTrack(id,kind,label,language,src,null);
						if (newTrack) {
						  if (!(videoElement.cTextTracks instanceof Array)) {
						    videoElement.cTextTracks = [];
						  }

						  videoElement.cTextTracks.push(newTrack);
	...
	captionator.decaptionify = function (videoElement) {
	  if (goog.dom.isElement(videoElement._containerObject)) {
	    goog.dom.removeNode(videoElement._containerObject);
	    videoElement._containerObject = null;
	  }
	};
	...
	/*
		...
		An array of CTextTrackCue Objects in initial state.
	*/
	...
	captionator.parseCaptions = function(captionData, options) {
		...
		if (captionData) {
			...
			var parseCaptionChunk = function parseCaptionChunk(subtitleElement,objectCount) {
				...
				// Turn back into string like the CTextTrackCue constructor expects
				cueSettings = "";
				...
				tmpCue = new captionator.CTextTrackCue(id, timeIn, timeOut, html, cueSettings, false, null);
			...
			var parseXMLChunk = function parseXMLChunk(xmlNode,index) {
				...
				return new captionator.CTextTrackCue(id, timeIn, timeOut, html, {}, false, null);
			};
	...
	captionator.processVideoElement = function (videoElement, defaultLanguage, options) {
		...
		if (!videoElement.captioned) {
			...
			[].slice.call(videoElement.querySelectorAll("track"),0).forEach(function(trackElement) {
				...
				var trackObject = videoElement.addCTextTrack(
				...
				trackElement.cTrack = trackObject;
				...
				if ((trackObject.kind === "subtitles" || trackObject.kind === "captions") &&
					(globalLanguage === trackObject.language)) {
					if (!trackList.filter(function(trackObject) {
							if ((trackObject.kind === "captions" || trackObject.kind === "subtitles") && globalLanguage === trackObject.language && trackObject.mode === captionator.CTextTrack.SHOWING) {
								return true;
							} else {
								return false;
							}
				...
				if (trackObject.kind === "chapters" && (globalLanguage === trackObject.language)) {
					if (!trackList.filter(function(trackObject) {
							if (trackObject.kind === "chapters" && trackObject.mode === captionator.CTextTrack.SHOWING) {
								return true;
							} else {
								return false;
							}
				...
				if (trackObject.kind === "descriptions" && (options.enableDescriptionsByDefault === true) && (globalLanguage === trackObject.language)) {
					if (!trackList.filter(function(trackObject) {
							if (trackObject.kind === "descriptions" && trackObject.mode === captionator.CTextTrack.SHOWING) {
								return true;
							} else {
								return false;
							}
				...
				if (trackEnabled === true) {
					trackList.forEach(function(trackObject) {
						if(trackObject.trackNode.hasAttribute("default") && trackObject.mode === captionator.CTextTrack.SHOWING) {
							trackObject.mode = captionator.CTextTrack.HIDDEN;
						}
					});
				}
				...
				if (trackEnabled === true) {
					trackObject.mode = captionator.CTextTrack.SHOWING;
				}
			});
			...
			videoElement.addEventListener("timeupdate", function(eventData){
				...
				// update active cues
				try {
					videoElement.cTextTracks.forEach(function(track) {
	...
	*/
	...
	/*
		...
		Second parameter: The CTextTrackCue itself.
	...
	*/
	...
	captionator.showTrackByIndex = function (videoElement, index) {
	  var trackList = videoElement.cTextTracks || [];
	  trackList.forEach(function (track, trackIndex) {
	    if (trackIndex == index) {
	      track.mode = captionator.CTextTrack.SHOWING;
	    } else {
	      track.mode = captionator.CTextTrack.OFF;
	    }
	  });
