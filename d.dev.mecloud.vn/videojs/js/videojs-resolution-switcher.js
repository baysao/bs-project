...
(function(window, videojs) {
  ...
  function setSourcesSanitized(player, sources) {
    ...
    var staticLabel = getElementFromArrayByClassName(_children, 'vjs-resolution-button-staticlabel');
     ...
     /*handleClick: function() {
      ...
      var self = this;
       window.addEventListener('mouseup',function(event) {
        var vjsMenu = document.getElementsByClassName("vjs-menu");
        if (event.target != vjsMenu[5] && event.target.parentNode != vjsMenu[5]){
          self.unpressButton();
          console.log(vjsMenu[5]);
        }
      });   
     },*/
  ...
  videoJsResolutionSwitcher = function(options) {
    ...
    player.updateSrc = function(src){
      ...
      player.controlBar.resolutionSwitcher = menuButton;

      player.controlBar.el_.insertBefore(menuButton.el_,player.controlBar.el_.querySelector(".vjs-playlist-button"))
      // player.controlBar.addChild(menuButton);
      //quick fix
      var menuButtonTmp = menuButton.el_;

      //custom quality UI
      var vjsTitleRes = player.controlBar.resolutionSwitcher.controlTextEl_;
      //var vjsMenuRes = player.controlBar.resolutionSwitcher.el_.children[0];
      //console.log(vjsMenuRes)
      //vjsMenuRes.insertBefore(vjsTitleRes,vjsMenuRes.children[0]);
